...
public class NeuralNetwork
{
	...
	public int Predict(double[] exampleData)
	{
		double[] xValues = new double[numInput]; // inputs
		double[] yValues; // computed Y

		Array.Copy(exampleData, xValues, numInput); // get x-values
		yValues = this.ComputeOutputs(xValues);
		int maxIndex = MaxIndex(yValues); // which cell in yValues has largest value?
	...
	public void saveInstance() {
		...
		FileStream file = File.Create("Assets//Other//neuralInstance.dat");

		NeuralData data = new NeuralData();
		data.numInput = numInput;
		data.numHidden = numHidden;
		data.numOutput = numOutput;

		data.inputs = inputs;
		data.ihWeights = ihWeights;
		data.hBiases = hBiases;
		data.hOutputs = hOutputs;

		data.hoWeights = hoWeights;
		data.oBiases = oBiases;
	...
	public void loadInstance() {
		Debug.Log(Application.dataPath + "//neuralInstance.dat");
		if(File.Exists(Application.dataPath + "//neuralInstance.dat")) {
			BinaryFormatter bf = new BinaryFormatter();
			FileStream file = File.Open(Application.dataPath + "//neuralInstance.dat", FileMode.Open);
			NeuralData data = (NeuralData)bf.Deserialize(file);
			file.Close();
			numInput = data.numInput;
			numHidden = data.numHidden;
			numOutput = data.numOutput;

			inputs = data.inputs;
			ihWeights = data.ihWeights;
			hBiases = data.hBiases;
			hOutputs = data.hOutputs;

			hoWeights = data.hoWeights;
			oBiases = data.oBiases;
			outputs = data.outputs;

			Debug.Log("Neural Network Loaded");
		}
	}
}
...
[Serializable]
class NeuralData {
	public int numInput; // number input nodes
